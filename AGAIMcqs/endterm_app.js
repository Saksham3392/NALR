// End Term App Controller
const QUIZ_QUESTIONS = (typeof window !== "undefined" && window.ENDTERM_QUIZ_QUESTIONS) 
  || (typeof ENDTERM_QUIZ_QUESTIONS !== "undefined" ? ENDTERM_QUIZ_QUESTIONS : []);
...
const STORAGE_KEY = "AGAI_ENDTERM_FINAL_STATE_V1";
let currentQIndex = 0;
let currentViewingMode = "question"; // "question" | "module_guide"
let isExamMode = false;
let activeGuideModuleId = "et_mod1";
...
function renderSidebarAccordion() {
  ...
  const currentActiveModId = currentViewingMode === "module_guide" 
    ? activeGuideModuleId 
    : (activeQuestion ? activeQuestion.module_id : "et_mod1");

  ENDTERM_SYLLABUS_MODULES.forEach((mod) => {
...
function renderModuleGuideView() {
  const guide = ENDTERM_MODULE_GUIDES[activeGuideModuleId] || ENDTERM_MODULE_GUIDES["et_mod1"];
  if (!guide) return;
  ...
  if (simContainer && typeof Visualizers !== "undefined") {
    ...
    if (activeGuideModuleId === "et_mod1" && Visualizers.renderRAGChunkingSimulator) {
      Visualizers.renderRAGChunkingSimulator("guideModSimContainer");
    } else if (activeGuideModuleId === "et_mod2" && Visualizers.renderTemperatureSimulator) {
      Visualizers.renderTemperatureSimulator("guideModSimContainer");
    } else if ((activeGuideModuleId === "et_mod3" || activeGuideModuleId === "et_mod4") && Visualizers.renderAgenticWorkflowSimulator) {
      Visualizers.renderAgenticWorkflowSimulator("guideModSimContainer");
    }
  }
...
function renderMainQuestionView() {
  ...
  if (nextBtn) {
    ...
    if (isLastQOfModule) {
      const currentModIdx = ENDTERM_SYLLABUS_MODULES.findIndex(m => m.id === currentModId);
      const nextModule = (currentModIdx !== -1 && currentModIdx < ENDTERM_SYLLABUS_MODULES.length - 1) ? ENDTERM_SYLLABUS_MODULES[currentModIdx + 1] : null;
      if (nextModule) {
...
function renderRoadmapModal() {
  ...
  ENDTERM_SYLLABUS_MODULES.forEach((mod, idx) => {
    const stats = getModuleStats(mod.id);
    ...
    card.innerHTML = `
      <div class="roadmap-timeline-marker">
        ...
        ${idx < ENDTERM_SYLLABUS_MODULES.length - 1 ? '<div class="roadmap-vertical-line"></div>' : ''}
      </div>
...
function showFinalResultsScreen() {
  ...
  let feedback = "";
  if (pct >= 85) {
    feedback = "🌟 Outstanding! You have thoroughly mastered all End Term Generative & Agentic AI topics!";
...
function setupEventListeners() {
  ...
  if (btnST1) {
    btnST1.classList.remove("active");
  ...
  if (btnEndTerm) {
    btnEndTerm.classList.add("active");
