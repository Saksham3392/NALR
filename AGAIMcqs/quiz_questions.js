// ST-2 Masterclass Question Bank (Lectures 24–37)
// 80 Complete Questions with Full Theory, Derivations & Drills
// Original 65 from GitHub + 15 Mock Test 1 additions
const QUIZ_QUESTIONS = [
  ...
  },
  {
    ...
    "theory": {
      "title": "Layer Normalization Mechanics in Transformer Blocks",
      "what_is_it": "Layer Normalization normalizes the activations across the feature channel dimension for a single token at a single timestep: μ = (1/d) Σ x_i, σ² = (1/d) Σ (x_i - μ)². It then scales and shifts using learned parameters γ and β.\n\n💡 Subject-Related Example:\nFor a token embedding vector with d_model = 4, x = [2.0, 4.0, 6.0, 8.0]. The mean is μ = 5.0, variance σ² = 5.0. Each component is normalized: (x_i - 5.0) / √(5.0 + ε) before applying learned parameters γ and β.\n\n🚀 Real-World AI Use Case:\nStandard Transformer architectures (Pre-LN in GPT-2/3/4 and LLaMA, Post-LN in original Vaswani et al.) rely on LayerNorm (or RMSNorm) to stabilize gradients and allow stacking 80+ layers without numerical explosion.",
      "why_we_need_it": "Batch Normalization requires computing statistics across the batch dimension, which causes major issues in sequence models with variable sequence lengths and small batch sizes during auto-regressive decoding. LayerNorm operates on each individual token independently, making inference and training completely consistent.",
      "how_it_works": "• Step 1: For a given token vector x ∈ ℝ^{d_model}, compute scalar mean μ across all d_model features.\n• Step 2: Compute scalar variance σ² across the d_model features.\n• Step 3: Standardize: x̂ = (x - μ) / √(σ² + ε).\n• Step 4: Apply element-wise learned affine transformation: y = γ ⊙ x̂ + β.",
      "formula": "LN(x) = \\frac{x - \\mu}{\\sqrt{\\sigma^2 + \\epsilon}} \\odot \\gamma + \\beta, \\quad \\text{where } \\mu, \\sigma^2 \\text{ are computed along } d_{model}",
      "key_takeaways": [
        "⭐ Sample & Token Independence: Normalizes across feature dimensions independently for each token position without looking at other batch samples.",
        "⭐ Batch Size Invariance: Works seamlessly with batch size 1 during auto-regressive generation.",
        "⭐ Gradient Stability: Prevents activation variance from exploding or collapsing through deep residual stacks."
      ]
    },
    "sample_questions": [
      {
        "q": "How does Layer Normalization differ from Batch Normalization in processing sequences?",
        "options": [
          "LayerNorm normalizes across feature channels for each token, while BatchNorm normalizes across batch items for each channel",
          "LayerNorm requires calculating statistics over the entire dataset",
          "LayerNorm can only be applied after the softmax layer",
          "BatchNorm does not require learned scale and shift parameters"
        ],
        "ans": "LayerNorm normalizes across feature channels for each token, while BatchNorm normalizes across batch items for each channel",
        "exp": "LayerNorm operates across hidden dimensions of an individual token, making it independent of batch size and other examples."
      },
      {
        "q": "Why is Batch Normalization impractical for autoregressive LLM token-by-token generation?",
        "options": [
          "During single-token inference (batch size = 1), batch statistics cannot be reliably computed",
          "BatchNorm removes positional encodings",
          "BatchNorm causes quadratic memory complexity",
          "BatchNorm prevents multi-head attention"
        ],
        "ans": "During single-token inference (batch size = 1), batch statistics cannot be reliably computed",
        "exp": "At inference time with batch size 1, batch variance is 0, making batch normalization unstable."
      }
    ]
  },
  {
    ...
    "module_name": "Module 3: Transformer Architecture Deep-Dive",
    "syllabus_lec": "Lectures 27–28",
    "topic": "Position-Wise Feed-Forward Sublayer Application",
    "difficulty": "Easy",
    "points": 1,
    "question": "Within a standard Transformer block, how is the feed-forward sublayer applied to a sequence after attention?",
    "options": [
      "The same feed-forward network is applied independently at each sequence position",
      "A different feed-forward network is trained for every sequence position",
      "The feed-forward network mixes all token positions through one dense matrix",
      "The feed-forward network is applied only to positions selected by the largest attention score"
    ],
    "correct": "The same feed-forward network is applied independently at each sequence position",
    "explanation": "The Position-Wise Feed-Forward Network (FFN) consists of two linear transformations with a non-linear activation (typically ReLU, GeLU, or SwiGLU) in between. It is applied to each token vector separately and identically. That means the exact same weight matrices (W₁, b₁ and W₂, b₂) are shared across all sequence positions, processing each token independently without mixing information between different token positions (the mixing of tokens occurs exclusively in the attention sublayer).",
    "theory": {
      "title": "Position-Wise Feed-Forward Network (FFN) Sublayer",
      "what_is_it": "In each Transformer block, the multi-head attention sublayer is followed by a fully connected feed-forward network. It is 'position-wise' because it processes each token position's representation vector independently and identically using the same weights.\n\n💡 Subject-Related Example:\nIf a sequence has 10 tokens [x₁, x₂, ..., x₁₀], each x_i ∈ ℝ^{d_model} passes through FFN(x_i) = W₂ · σ(W₁ · x_i + b₁) + b₂. The weights W₁ (d_model × d_ff) and W₂ (d_ff × d_model) are identical for token 1, token 2, ..., token 10.\n\n🚀 Real-World AI Use Case:\nResearchers have shown that the FFN sublayers in Transformers act as key-value associative memories that store factual knowledge (e.g., associating 'Eiffel Tower' with 'Paris').",
      "why_we_need_it": "Self-attention only performs linear combinations of value vectors across positions. The FFN introduces non-linear feature transformations and expands representation capacity (typically expanding to d_ff = 4 × d_model, e.g., 512 → 2048 → 512).",
      "how_it_works": "• Step 1: Receives post-attention representation x for each token.\n• Step 2: First linear projection expands dimension from d_model to d_ff (usually 4 × d_model).\n• Step 3: Apply non-linear activation function σ (ReLU, GELU, or Swish).\n• Step 4: Second linear projection compresses dimension back from d_ff to d_model.",
      "formula": "\\text{FFN}(x) = \\max(0, xW_1 + b_1)W_2 + b_2 \\quad \\text{or} \\quad \\text{FFN}(x) = \\text{GELU}(xW_1 + b_1)W_2 + b_2",
      "key_takeaways": [
        "⭐ Shared Parameters: The identical weights W₁, W₂ are applied at every position.",
        "⭐ No Cross-Token Mixing: Operates strictly within each token vector; cross-token communication is handled by attention.",
        "⭐ Dimensional Expansion: Standard inner dimension d_ff is 4 times d_model (e.g., 768 → 3072 → 768)."
      ]
    },
    "sample_questions": [
      {
        "q": "Does the Position-Wise Feed-Forward Network mix information between token position 3 and token position 7?",
        "options": [
          "No, it operates strictly independently on each individual token position vector",
          "Yes, via the intermediate non-linear activation",
          "Yes, through matrix transpose operations",
          "Only in the decoder blocks"
        ],
        "ans": "No, it operates strictly independently on each individual token position vector",
        "exp": "The FFN is strictly position-wise: each token vector is transformed in isolation using shared weights."
      },
      {
        "q": "What is the typical ratio of the FFN hidden layer dimension (d_ff) to the model embedding dimension (d_model)?",
        "options": [
          "Typically 4:1 (d_ff = 4 × d_model)",
          "Typically 1:1",
          "Typically 1:4",
          "Typically 8:1"
        ],
        "ans": "Typically 4:1 (d_ff = 4 × d_model)",
        "exp": "Standard transformer blocks expand the hidden dimension to 4 × d_model (e.g., 768 to 3072 in BERT-base)."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "module_name": "Module 4: Positional Encoding & Sequence Modeling",
    "syllabus_lec": "Lectures 29–30",
    "topic": "Position-Dependent Encodings for Order Disambiguation",
    "difficulty": "Easy",
    "points": 1,
    "question": "Two sequences contain exactly the same token embeddings but in different orders. Which addition allows a self-attention model to distinguish their positions before attention is computed?",
    "options": [
      "Position-dependent encodings added to token representations",
      "A larger vocabulary projection applied after the final layer",
      "A shared dropout mask applied to every token embedding",
      "A larger number of value vectors with unchanged token representations"
    ],
    "correct": "Position-dependent encodings added to token representations",
    "explanation": "Self-attention is fundamentally permutation-equivariant: without positional encodings, the attention calculation between tokens depends only on their content, not their order. Thus, 'dog bites man' and 'man bites dog' would produce identical sets of representations. Adding position-dependent encodings (either sinusoidal functions or learned position embeddings) directly to token embeddings injects distinct coordinate information, allowing the model to distinguish order.",
    "theory": {
      "title": "Positional Encodings & Sequence Order Representation",
      "what_is_it": "Positional encodings are vectors added element-wise to token embeddings (input = token_embedding + pos_encoding) before feeding into the first Transformer block. They assign a unique signature to each sequence position.\n\n💡 Subject-Related Example:\n'The cat ate the mouse' vs 'The mouse ate the cat'. Both sentences have identical word embeddings. By adding PE(pos=1) to 'cat' in sentence 1, and PE(pos=4) to 'cat' in sentence 2, the model knows in sentence 1 that 'cat' is the subject/actor, while in sentence 2 it is the object.\n\n🚀 Real-World AI Use Case:\nUsed across all modern sequence architectures. GPT models use learned positional embeddings; original Transformer uses fixed sinusoidal encodings; modern models like RoFormer and LLaMA use Rotary Position Embeddings (RoPE).",
      "why_we_need_it": "Self-attention computes dot products between pairs of vectors regardless of where they appear in the sequence. Without positional encodings, self-attention treats a sentence as an unordered bag of words.",
      "how_it_works": "• Step 1: For each position pos ∈ [0, N-1] and each dimension 2i / 2i+1 up to d_model:\n• Step 2: Compute sine and cosine waves of geometrically increasing wavelengths.\n• Step 3: Add this positional vector directly to the token's semantic embedding vector.",
      "formula": "PE_{(pos, 2i)} = \\sin\\left(\\frac{pos}{10000^{2i/d_{model}}}\\right), \\quad PE_{(pos, 2i+1)} = \\cos\\left(\\frac{pos}{10000^{2i/d_{model}}}\\right)",
      "key_takeaways": [
        "⭐ Permutation Equivariance Solution: Injects order awareness into otherwise order-agnostic attention.",
        "⭐ Direct Addition: Positional encodings are added element-wise to input token embeddings (same dimension d_model).",
        "⭐ Relative Position Generalization: Sinusoidal encodings allow the model to easily learn relative positions because PE_{pos+k} can be expressed as a linear function of PE_{pos}."
      ]
    },
    "sample_questions": [
      {
        "q": "Why can a raw self-attention mechanism not differentiate between 'A B C' and 'C B A' without positional encoding?",
        "options": [
          "Because self-attention is permutation-equivariant; it treats sequences as an unordered set of token vectors",
          "Because softmax forces all tokens to be positive",
          "Because query and key vectors cancel each other out",
          "Because dot products are only computed for adjacent tokens"
        ],
        "ans": "Because self-attention is permutation-equivariant; it treats sequences as an unordered set of token vectors",
        "exp": "Without positional encodings, self-attention has no information about the order of tokens in the sequence."
      },
      {
        "q": "How are positional encodings incorporated into the Transformer input?",
        "options": [
          "Added element-wise directly to the token embedding vectors",
          "Concatenated to the end of the sequence as extra tokens",
          "Multiplied with the vocabulary projection matrix",
          "Applied only to the final output logits"
        ],
        "ans": "Added element-wise directly to the token embedding vectors",
        "exp": "Standard transformers perform element-wise addition: X = TokenEmbedding(x) + PositionalEncoding."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "module_name": "Module 5: Vision Transformers (ViT, Swin, CaiT)",
    "syllabus_lec": "Lecture 31",
    "topic": "ViT Patch Sequence Length Calculation (192×192 Image with [CLS] Token)",
    "difficulty": "Medium",
    "points": 1,
    "question": "A Vision Transformer splits a 192 × 192 image into non-overlapping 16 × 16 patches and prepends one class token. What sequence length enters the encoder?",
    "options": [
      "144",
      "145",
      "192",
    ...
    "correct": "145",
    "explanation": "To calculate sequence length in a Vision Transformer (ViT):\n1. Number of vertical patches: H / P = 192 / 16 = 12.\n2. Number of horizontal patches: W / P = 192 / 16 = 12.\n3. Total visual patches N = (H / P) × (W / P) = 12 × 12 = 144.\n4. Adding the 1 prepended learnable class token ([CLS]) gives total sequence length = N + 1 = 144 + 1 = 145.",
    "theory": {
      "title": "Patch Partitioning & Sequence Length in Vision Transformers",
      "what_is_it": "A standard Transformer expects a 1D sequence of token embeddings. To handle 2D images, ViT partitions an image of size H × W × C into a grid of non-overlapping patches of size P × P. These N patches are flattened, linearly projected, and prepended with a learnable [CLS] token.\n\n💡 Subject-Related Example:\nFor a 224 × 224 image with 16 × 16 patches: N = (224/16) × (224/16) = 14 × 14 = 196 patches. With [CLS], sequence length is 197. For a 192 × 192 image with 16 × 16 patches: N = 12 × 12 = 144 patches. With [CLS], sequence length is 145.\n\n🚀 Real-World AI Use Case:\nViT-Base, CLIP visual backbones, and DINOv2 all partition images into patches (usually 14×14 or 16×16) and process them as token sequences with a [CLS] classification token.",
      "why_we_need_it": "Self-attention on individual pixels of a 192×192 image would yield N = 36,864 tokens, requiring a 36,864 × 36,864 attention matrix (1.35 billion elements per layer!), causing GPU out-of-memory. Patching compresses the sequence to 144 tokens.",
      "how_it_works": "• Step 1: Compute grid dimensions: N_H = H / P, N_W = W / P.\n• Step 2: Total image patches: N = N_H × N_W.\n• Step 3: Add 1 class token: Total Sequence Length = N + 1.\n• Step 4: Each patch is flattened to dimension P × P × C and linearly projected to d_model.",
      "formula": "\\text{Sequence Length} = \\left(\\frac{H}{P} \\times \\frac{W}{P}\\right) + 1 = \\left(\\frac{192}{16} \\times \\frac{192}{16}\\right) + 1 = 12 \\times 12 + 1 = 144 + 1 = 145",
      "key_takeaways": [
        "⭐ Formula: N = (H/P) × (W/P). Always remember to add 1 for the [CLS] token!",
        "⭐ Calculation: 192 / 16 = 12; 12² = 144; 144 + 1 = 145.",
        "⭐ Token Meaning: The [CLS] token aggregates global representation through self-attention layers for final classification."
      ]
    },
    "sample_questions": [
      {
        "q": "If a 256 × 256 image is split into 16 × 16 patches in a ViT with 1 class token, what is the encoder sequence length?",
        "options": [
          "257",
          "256",
          "128",
        ...
        "ans": "257",
        "exp": "(256/16) × (256/16) + 1 = 16 × 16 + 1 = 256 + 1 = 257."
      },
      {
        "q": "What is the role of the prepended class token ([CLS]) in ViT?",
        "options": [
          "To collect and aggregate whole-image visual information via self-attention for the classification head",
          "To store the average color of the image",
          "To provide positional coordinates for the image border",
          "To normalize the patch activations"
        ],
        "ans": "To collect and aggregate whole-image visual information via self-attention for the classification head",
        "exp": "The [CLS] token's state at the final layer serves as the aggregate representation vector for the entire image."
      }
    ]
  },
  {
    ...
    "module_name": "Module 5: Vision Transformers (ViT, Swin, CaiT)",
    "syllabus_lec": "Lecture 31",
    "topic": "Swin Transformer Shifted Window Mechanism",
    "difficulty": "Medium",
    "points": 1,
    "question": "What is the main purpose of shifted windows in a Swin Transformer compared with repeatedly using the same fixed local windows?",
    "options": [
      "Allow information exchange across neighbouring window boundaries while retaining local attention",
      "Convert every local window into one global attention matrix at each layer",
      "Remove the need for patch embeddings by shifting raw pixels directly",
      "Keep tokens permanently isolated inside the same local window through all layers"
    ],
    "correct": "Allow information exchange across neighbouring window boundaries while retaining local attention",
    "explanation": "Standard Vision Transformers compute global self-attention across all image patches with quadratic complexity O(N²), which is computationally prohibitive for high-resolution images. Swin Transformer limits attention to non-overlapping local windows (linear complexity O(N)). However, if windows remained fixed, tokens in adjacent windows could never communicate. By shifting window partitions by (⌊M/2⌋, ⌊M/2⌋) pixels between consecutive layers, Swin Transformer introduces cross-window connections, allowing information to propagate across the entire image while preserving linear computational efficiency.",
    "theory": {
      "title": "Shifted Window Self-Attention in Swin Transformers",
      "what_is_it": "Swin Transformer computes local self-attention within partitioned windows of size M × M (e.g. M = 7). In layer l, standard window partitioning (W-MSA) is used. In layer l+1, the window partitioning is shifted by (⌊M/2⌋, ⌊M/2⌋) patches (SW-MSA). This connects adjacent windows from the previous layer.\n\n💡 Subject-Related Example:\nImagine a chessboard divided into 2×2 squares. If pieces can only look at other pieces inside their own 2×2 square, pieces near the edge of square A cannot see pieces across the line in square B. Shifting the grid by 1 square to the right and down merges parts of four adjacent squares into new windows, enabling instant cross-border communication!\n\n🚀 Real-World AI Use Case:\nSwin Transformer is the backbone for high-resolution dense computer vision tasks like object detection (Mask R-CNN), semantic segmentation (ADE20K), and medical imaging where full global attention would exceed GPU VRAM.",
      "why_we_need_it": "Global attention on an 800×800 image produces thousands of patches, making O(N²) attention intractable. Local windows reduce complexity to linear O(M² · N), but without shifting, features across borders would remain forever isolated.",
      "how_it_works": "• Step 1 (Layer l): Regular Window Attention (W-MSA) computes attention strictly within M × M local patches.\n• Step 2 (Layer l+1): Shift the window grid by (⌊M/2⌋, ⌊M/2⌋) patches.\n• Step 3: Use cyclic shifting and masking to maintain efficient batch computation without increasing window count.\n• Step 4: Patches from neighbouring previous windows now reside in the same window, exchanging cross-boundary information.",
      "formula": "\\text{Complexity: } \\Omega(\\text{Global}) = 4hwC^2 + 2(hw)^2C \\quad \\text{vs.} \\quad \\Omega(\\text{Swin}) = 4hwC^2 + 2M^2(hw)C",
      "key_takeaways": [
        "⭐ Cross-Window Information: Shifting windows bridges neighbouring local windows, enabling cross-boundary context.",
        "⭐ Linear Complexity: Retains O(N) linear computational complexity relative to image size rather than O(N²) quadratic.",
        "⭐ Alternating Design: Layer l uses regular local windows (W-MSA); Layer l+1 uses shifted windows (SW-MSA)."
      ]
    },
    "sample_questions": [
      {
        "q": "What is the computational complexity of local window self-attention with respect to image token count N?",
        "options": [
          "Linear O(N), because window size M is fixed",
          "Quadratic O(N²)",
          "Logarithmic O(log N)",
          "Exponential O(2^N)"
        ],
        "ans": "Linear O(N), because window size M is fixed",
        "exp": "Because each window size M × M is constant, computing attention within all windows scales linearly with the total number of patches N."
      },
      {
        "q": "What would happen in a Swin Transformer if shifted window attention was never used?",
        "options": [
          "Information would remain permanently trapped inside isolated local windows with zero cross-boundary feature exchange",
          "The model would become a pure CNN",
          "The sequence length would double at each layer",
          "The patch size would collapse to zero"
        ],
        "ans": "Information would remain permanently trapped inside isolated local windows with zero cross-boundary feature exchange",
        "exp": "Without shifting, tokens in different windows would never be able to attend to one another."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "module_id": "mod6",
    "module_name": "Module 6: LLM Architecture & Autoregressive Pre-training",
    "syllabus_lec": "Lectures 33–34",
    "topic": "Autoregressive Pre-Training Objective (Next-Token Prediction)",
    "difficulty": "Easy",
    "points": 1,
    "question": "During GPT-style decoder-only pre-training, what prediction objective is applied at each eligible token position?",
    "options": [
      "Predict the next token from the preceding context",
      "Predict the preceding token from the following context",
      "Predict a masked token using unrestricted bidirectional context",
      "Predict the complete sequence label from the first token only"
    ],
    "correct": "Predict the next token from the preceding context",
    "explanation": "GPT-style decoder-only models are trained using causal autoregressive language modeling (next-token prediction). Given a sequence of prefix tokens (x₁, x₂, ..., x_{t-1}), the objective at position t is to predict the probability distribution of the next token x_t. To prevent looking ahead into future tokens during training, a lower-triangular causal attention mask is applied, ensuring token t can only attend to positions ≤ t.",
    "theory": {
      "title": "Autoregressive Next-Token Prediction Objective",
      "what_is_it": "Autoregressive language modeling factorizes the joint probability of a sequence of tokens into a product of conditional probabilities using the probability chain rule: P(x₁, ..., x_T) = ∏_{t=1}^T P(x_t | x₁, ..., x_{t-1}). At every token position, the model outputs logits over the vocabulary, trained with cross-entropy loss against the true next token.\n\n💡 Subject-Related Example:\nGiven the prompt 'The capital of France is', the model computes hidden state at the last token and predicts a high probability for token 'Paris'. In training, this is computed for all positions simultaneously in parallel using teacher forcing and causal masking.\n\n🚀 Real-World AI Use Case:\nThe foundation of all modern generative foundation models: GPT-4o, LLaMA-3, Mistral, Gemini, Claude, and DeepSeek.",
      "why_we_need_it": "Next-token prediction is an unsupervised (self-supervised) task that can be trained on trillions of tokens of raw Internet text without requiring human labeling, creating general world models with reasoning capabilities.",
      "how_it_works": "• Step 1: Pass input sequence [x₁, ..., x_T] through embedding and decoder layers.\n• Step 2: Causal mask sets upper-triangular attention logits to -∞ so position t cannot attend to t+1.\n• Step 3: Hidden states [h₁, ..., h_{T-1}] are projected to vocabulary dimension via LM Head.\n• Step 4: Target sequence is the original input shifted left by 1 position: targets = [x₂, ..., x_T].\n• Step 5: Compute cross-entropy loss: mathcal{L} = -sum log P(x_{t+1} | x_{1:t}).",
      "formula": "\\mathcal{L}_{\\text{NLL}} = -\\frac{1}{T} \\sum_{t=1}^{T} \\log P(x_t \\mid x_1, x_2, \\ldots, x_{t-1}; \\Theta)",
      "key_takeaways": [
        "⭐ Unidirectional Causal Context: Predicts token t exclusively from preceding tokens x_{1:t-1}.",
        "⭐ Parallel Training: Causal masking enables parallel training across all token positions in one forward pass (Teacher Forcing).",
        "⭐ Contrast with BERT: BERT uses Masked Language Modeling (bidirectional context), whereas GPT uses causal autoregressive prediction."
      ]
    },
    "sample_questions": [
      {
        "q": "How does GPT train on next-token prediction in parallel rather than token-by-token?",
        "options": [
          "By applying a lower-triangular causal attention mask to prevent future token information leakage",
          "By training a separate neural network for each word",
          "By reversing the sentence order",
          "By predicting all words simultaneously with no masking"
        ],
        "ans": "By applying a lower-triangular causal attention mask to prevent future token information leakage",
        "exp": "Causal masking blocks future tokens, allowing the entire sequence to be processed in parallel during training via teacher forcing."
      },
      {
        "q": "What loss function is used for autoregressive language model training?",
        "options": [
          "Cross-Entropy / Negative Log-Likelihood (NLL) loss over the vocabulary",
          "Mean Squared Error (MSE)",
          "Hinge Loss",
          "Contrastive InfoNCE Loss"
        ],
        "ans": "Cross-Entropy / Negative Log-Likelihood (NLL) loss over the vocabulary",
        "exp": "Cross-entropy loss compares the model's predicted next-token probability distribution with the one-hot ground-truth token."
      }
    ]
  },
  {
    ...
    "module_name": "Module 6: LLM Architecture & Autoregressive Pre-training",
    "syllabus_lec": "Lectures 33–34",
    "topic": "Pre-Training Cross-Entropy / NLL Loss Calculation",
    "difficulty": "Hard",
    "points": 1,
    "question": "During decoder-only pre-training, the model assigns probabilities 0.5 and 0.25 to the correct next tokens at two positions. Using natural-log loss and ln(2) ≈ 0.693, what is the mean negative log-likelihood?",
    "options": [
      "0.693",
      "1.040",
      "1.386",
    ...
    "correct": "1.040",
    "explanation": "Negative Log-Likelihood (NLL) loss for next-token prediction is computed as:\nL = -ln(P(target_token))\n\n1. For position 1: P₁ = 0.5 = 1/2.\n   Loss₁ = -ln(0.5) = -ln(2⁻¹) = ln(2) ≈ 0.693.\n\n2. For position 2: P₂ = 0.25 = 1/4 = (1/2)².\n   Loss₂ = -ln(0.25) = -ln(2⁻²) = 2 · ln(2) ≈ 2 · 0.693 = 1.386.\n\n3. Mean Negative Log-Likelihood across the two positions:\n   Mean Loss = (Loss₁ + Loss₂) / 2 = (0.693 + 1.386) / 2 = 2.079 / 2 = 1.0395 ≈ 1.040.",
    "theory": {
      "title": "Cross-Entropy & Negative Log-Likelihood (NLL) Loss in LLMs",
      "what_is_it": "Cross-entropy loss in autoregressive language modeling measures how well the predicted probability distribution aligns with the one-hot target token. For a sequence of tokens, it equals the mean negative natural logarithm of the probability assigned to the ground-truth next token: \\mathcal{L} = -\\frac{1}{N} \\sum_{i=1}^N \\ln(P(y_i)).\n\n💡 Subject-Related Example:\nIf the model is 100% confident in the correct token (P = 1.0), Loss = -ln(1.0) = 0 (perfect!). If the model assigns P = 0.5, Loss = ln(2) ≈ 0.693. If P = 0.25, Loss = ln(4) = 2 · ln(2) ≈ 1.386. If P → 0, Loss → ∞ (heavy penalty for being wrong!).\n\n🚀 Real-World AI Use Case:\nUsed as the primary pre-training loss for GPT-4, LLaMA-3, and Claude. Perplexity (PPL) is directly derived as the exponent of NLL loss: PPL = \\exp(\\mathcal{L}).",
      "why_we_need_it": "NLL loss severely penalizes overconfident incorrect predictions and rewards confident correct predictions, providing steep gradients that push the model toward accurate token distribution modeling.",
      "how_it_works": "• Step 1: For each token position i, calculate loss: L_i = -\\ln(P_i).\n• Step 2: Use logarithm rules: -\\ln(0.5) = \\ln(2) \\approx 0.693; -\\ln(0.25) = \\ln(4) = 2\\ln(2) \\approx 1.386.\n• Step 3: Sum the individual token losses: 0.693 + 1.386 = 2.079.\n• Step 4: Divide by total token count (N = 2): 2.079 / 2 = 1.0395 \\approx 1.040.",
      "formula": "\\mathcal{L}_{\\text{mean}} = \\frac{1}{2} \\left( -\\ln(0.5) + (-\\ln(0.25)) \\right) = \\frac{\\ln(2) + 2\\ln(2)}{2} = \\frac{3 \\times 0.693}{2} = 1.0395 \\approx 1.040",
      "key_takeaways": [
        "⭐ Mathematical Property: -ln(0.5) = ln(2) ≈ 0.693; -ln(0.25) = 2 · ln(2) ≈ 1.386.",
        "⭐ Mean Calculation: (0.693 + 1.386) / 2 = 2.079 / 2 = 1.040.",
        "⭐ Perplexity Relation: Perplexity = e^{Loss}. An NLL loss of 1.040 corresponds to a perplexity of e^{1.040} ≈ 2.83."
      ]
    },
    "sample_questions": [
      {
        "q": "If a model predicts the correct next token with probability 0.5, what is its negative log-likelihood loss given ln(2) = 0.693?",
        "options": [
          "0.693",
          "0.500",
          "1.000",
        ...
        "ans": "0.693",
        "exp": "Loss = -ln(0.5) = -ln(2⁻¹) = ln(2) ≈ 0.693."
      },
      {
        "q": "What is the relationship between Cross-Entropy Loss L and Perplexity (PPL)?",
        "options": [
          "Perplexity = exp(L)",
          "Perplexity = log(L)",
          "Perplexity = 1 / L",
        ...
        "ans": "Perplexity = exp(L)",
        "exp": "Perplexity is the exponential of cross-entropy loss: PPL = e^L."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "module_id": "mod7",
    "module_name": "Module 7: Prompt Engineering Fundamentals",
    "syllabus_lec": "Lecture 35",
    "topic": "Zero-Shot Prompting Foundations",
    "difficulty": "Easy",
    "points": 1,
    "question": "An LLM is given only a task instruction and a new input, with no worked examples in the prompt. Which prompting setup is being used?",
    "options": [
      "Zero-shot prompting",
      "Few-shot prompting",
    ...
    "correct": "Zero-shot prompting",
    "explanation": "In Zero-Shot Prompting, the language model is presented with an instruction describing the task and the input instance to be processed, with ZERO demonstration examples (0-shot). The model must rely entirely on the general world knowledge, instruction-following capabilities, and semantic representations acquired during its pre-training and instruction-tuning (RLHF) phases.",
    "theory": {
      "title": "Zero-Shot Prompting in Generative AI",
      "what_is_it": "Zero-shot prompting tests whether a foundation model can generalize to an unseen task purely from natural language instructions without providing input-output demonstration pairs in the prompt context.\n\n💡 Subject-Related Example:\nPrompt: 'Classify the sentiment of the following review as Positive or Negative: \"The battery life exceeded my expectations.\" Sentiment:' -> Model immediately responds: 'Positive'. No prior examples were given.\n\n🚀 Real-World AI Use Case:\nEveryday conversational queries to ChatGPT, Claude, and Gemini where users simply ask a question or command without providing training pairs.",
      "why_we_need_it": "Zero-shot prompting is the fastest, simplest, and most token-efficient prompting technique, requiring no curated dataset of examples and consuming minimal context window space.",
      "how_it_works": "• Step 1: System prompt defines the persona and task scope.\n• Step 2: Clear instruction directive specifies the exact transformation required.\n• Step 3: Input text is provided directly.\n• Step 4: Model autoregressively completes the prompt based on internal learned weights.",
      "formula": "\\text{Prompt} = [\\text{Instruction}] + [\\text{Input Data}] \\longrightarrow \\text{Output}",
      "key_takeaways": [
        "⭐ 0 Worked Examples: Contains instructions and input, but zero worked demonstrations.",
        "⭐ Token Efficiency: Minimizes context length and reduces latency and API costs.",
        "⭐ Capability Threshold: Relies heavily on instruction fine-tuning (RLHF / SFT) of the underlying base LLM."
      ]
    },
    "sample_questions": [
      {
        "q": "What is the primary advantage of zero-shot prompting over few-shot prompting?",
        "options": [
          "Consumes significantly fewer context tokens, reducing API costs and latency",
          "Guarantees 100% factual accuracy",
          "Updates model weights dynamically",
          "Bypasses context window limits completely"
        ],
        "ans": "Consumes significantly fewer context tokens, reducing API costs and latency",
        "exp": "Zero-shot prompts omit demonstration examples, keeping prompt length short and fast to execute."
      },
      {
        "q": "What distinguishes zero-shot prompting from fine-tuning?",
        "options": [
          "Zero-shot prompting updates zero model weights; it operates entirely at inference time",
          "Zero-shot prompting requires a larger training dataset",
          "Zero-shot prompting only runs on GPUs",
          "Fine-tuning does not change model weights"
        ],
        "ans": "Zero-shot prompting updates zero model weights; it operates entirely at inference time",
        "exp": "Prompting changes only the context input; the model parameters remain completely frozen."
      }
    ]
  },
  {
    ...
    "module_name": "Module 7: Prompt Engineering Fundamentals",
    "syllabus_lec": "Lecture 35",
    "topic": "Few-Shot In-Context Demonstration Prompting",
    "difficulty": "Easy",
    "points": 1,
    "question": "A prompt includes two labelled examples before asking the model to solve a new case in the same format. Which prompting technique is primarily being used?",
    "options": [
      "Zero-shot prompting",
      "Few-shot prompting",
      "Full fine-tuning",
    ...
    "explanation": "Few-Shot Prompting (In-Context Learning) provides a small number (typically 2 to 5) of worked input-output demonstration examples directly inside the prompt context before presenting the target problem. This primes the model's attention mechanism to recognize and follow the exact task format, stylistic tone, and reasoning structure without updating any weights.",
    "theory": {
      "title": "Few-Shot Prompting & In-Context Learning",
      "what_is_it": "Few-shot prompting conditions an autoregressive language model on exemplar pairs (Input_1 -> Output_1, Input_2 -> Output_2) within the context window. The model recognizes the repeating pattern and continues the generation for the query input in identical style and formatting.\n\n💡 Subject-Related Example:\n'Review: Great food! -> Sentiment: Positive\nReview: Service was terribly slow. -> Sentiment: Negative\nReview: The screen shattered on day one. -> Sentiment:' -> Model immediately completes: 'Negative'.\n\n🚀 Real-World AI Use Case:\nEnforcing strict structured outputs (like custom JSON schemas, code generation patterns, domain-specific classification) without fine-tuning model weights.",
      "why_we_need_it": "Natural language instructions can be ambiguous. Showing concrete input-output examples eliminates ambiguity, demonstrates edge-case handling, and stabilizes output formatting.",
      "how_it_works": "• Step 1: Provide general instruction.\n• Step 2: Append 1 to 5 exemplar pairs: Exemplar (Input, Output).\n• Step 3: Present query input and leave target output position open for generation.\n• Step 4: Self-attention heads attend to the demonstration tokens to guide output probabilities.",
      "formula": "\\text{Prompt} = [\\text{Task Definition}] + (x_1, y_1) + (x_2, y_2) + \\dots + (x_k, y_k) + x_{\\text{target}} \\longrightarrow y_{\\text{target}}",
      "key_takeaways": [
        "⭐ In-Context Exemplars: Contains 2+ worked demonstrations showing input-to-output mapping.",
        "⭐ Zero Weight Updates: Completely gradient-free; adaptation happens entirely in forward-pass activations.",
        "⭐ Format Adherence: Drastically reduces structural errors, hallucinations, and formatting deviations."
      ]
    },
    "sample_questions": [
      {
        "q": "Does few-shot in-context prompting perform gradient descent updates on model parameters?",
        "options": [
          "No, model parameters remain completely frozen",
          "Yes, on the final projection layer only",
          "Yes, on attention query matrices only",
          "Only when using more than 5 examples"
        ],
        "ans": "No, model parameters remain completely frozen",
        "exp": "In-context learning utilizes the model's existing attention mechanism to follow patterns without any parameter updates."
      },
      {
        "q": "Why does few-shot prompting often outperform zero-shot prompting on specialized formatting tasks?",
        "options": [
          "Demonstrations provide unambiguous concrete patterns that guide attention heads to reproduce the exact syntax",
          "It increases the model's vocabulary size",
          "It forces temperature to zero",
          "It converts the decoder into an encoder"
        ],
        "ans": "Demonstrations provide unambiguous concrete patterns that guide attention heads to reproduce the exact syntax",
        "exp": "Exemplars clarify formatting requirements that are difficult to specify completely in abstract natural language."
      }
    ]
  },
  {
    ...
    "module_name": "Module 7: Prompt Engineering Fundamentals",
    "syllabus_lec": "Lecture 35",
    "topic": "Prompt Formatting & Output Template Design",
    "difficulty": "Easy",
    "points": 1,
    "question": "A prompting experiment gives correct content but inconsistent output structure across inputs. The model weights must remain unchanged. Which modification most directly improves format consistency while staying within prompt engineering?",
    "options": [
      "Provide a precise output template together with a few representative formatted examples",
      "Increase the number of trainable adapter parameters without changing the prompt",
      "Replace retrieval embeddings while keeping the prompt unchanged",
      "Continue decoder pre-training on unrelated text before each request"
    ],
    "correct": "Provide a precise output template together with a few representative formatted examples",
    "explanation": "When an LLM produces semantically correct answers but inconsistent output structure (such as varying JSON keys, differing markdown headers, or irregular delimiters), the most effective prompt engineering solution is to supply an explicit output format template (e.g., specifying exact field names and types) combined with a few formatted demonstration examples (few-shot prompting). This constrains the model's token sampling distribution to adhere strictly to the target syntactic structure without touching model weights.",
    "theory": {
      "title": "Output Template Design & Format Enforcement in Prompting",
      "what_is_it": "Output template design is a prompt engineering methodology where the system prompt explicitly specifies the required structural schema (such as JSON, XML, or Markdown tables) and includes 1-2 few-shot exemplars demonstrating the exact formatting.\n\n💡 Subject-Related Example:\nInstead of asking: 'Extract names and emails from this text', prompt with:\n'Return a valid JSON object matching this exact schema:\n{\n  \"name\": \"string\",\n  \"email\": \"string\"\n}\nExample:\nInput: Alice at alice@gmail.com\nOutput: {\"name\": \"Alice\", \"email\": \"alice@gmail.com\"}'\n\n🚀 Real-World AI Use Case:\nEssential for building production AI agents, LangChain tools, and backend API integrations that require predictable JSON output parsing without runtime crashes.",
      "why_we_need_it": "Downstream software pipelines cannot parse unpredictable free-form text. Unstructured responses break automated parsers. Providing templates and examples eliminates structural variation.",
      "how_it_works": "• Step 1: Define explicit JSON / XML schema in the instruction.\n• Step 2: Use delimiters (e.g., ```json ... ```) to indicate output boundaries.\n• Step 3: Provide 1-2 few-shot exemplars showing the exact expected syntax.\n• Step 4: Prefix the assistant generation prompt with the opening delimiter (e.g., '{') to anchor the token distribution.",
      "formula": "\\text{Structured Prompt} = \\text{Instruction} + \\text{Output Schema Template} + \\text{Few-Shot Exemplars} + \\text{Input}",
      "key_takeaways": [
        "⭐ Prompt-Only Solution: Enforces structural consistency without modifying model weights or training adapters.",
        "⭐ Schema Specification: Explicitly stating keys and formatting prevents syntactic variability.",
        "⭐ Synergistic Few-Shot: Combining schema instructions with concrete exemplars provides the highest reliability."
      ]
    },
    "sample_questions": [
      {
        "q": "Why is providing an explicit JSON schema template in a prompt more effective than simply saying 'format as JSON'?",
        "options": [
          "Because explicit schema templates define exact key names and data types, eliminating ambiguity",
          "Because JSON schema compiles the prompt into C++ code",
          "Because models only understand JSON syntax",
          "Because it reduces token count to zero"
        ],
        "ans": "Because explicit schema templates define exact key names and data types, eliminating ambiguity",
        "exp": "Without a schema, an LLM might name a key 'user_name', 'name', or 'username' unpredictably."
      },
      {
        "q": "Can structured output formatting be achieved without fine-tuning model weights?",
        "options": [
          "Yes, via output template design and few-shot formatting examples within prompt engineering",
          "No, weights must always be updated to output JSON",
          "Only if the LLM is connected to an external SQL database",
          "Only in classification tasks"
        ],
        "ans": "Yes, via output template design and few-shot formatting examples within prompt engineering",
        "exp": "Prompt formatting templates guide autoregressive token generation without requiring parameter updates."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "module_name": "Module 8: Fine-Tuning & Parameter-Efficient Adaptation (LoRA & PEFT)",
    "syllabus_lec": "Lectures 36–37",
    "topic": "LoRA Multi-Task Adapter Architecture with Frozen Base Model",
    "difficulty": "Medium",
    "points": 1,
    "question": "A team wants separate lightweight task adaptations while keeping one shared LLM checkpoint unchanged. Which approach most closely matches LoRA-based PEFT?",
    "options": [
      "Train low-rank update matrices while freezing the base model weights",
      "Duplicate and fully fine-tune every base-model weight for each task",
      "Update only token embeddings while permanently removing attention projections",
      "Retrain the language model from random initialization for each task"
    ],
    "correct": "Train low-rank update matrices while freezing the base model weights",
    "explanation": "Low-Rank Adaptation (LoRA) is a Parameter-Efficient Fine-Tuning (PEFT) technique that freezes the pre-trained base model weights W₀ and decomposes the weight update into two low-rank trainable matrices: ΔW = B · A, where B ∈ ℝ^{d × r} and A ∈ ℝ^{r × k} with rank r ≪ min(d, k). This allows serving dozens of customized task adapters (each taking only tens of megabytes of disk space) on top of a single shared, frozen multi-gigabyte foundation model checkpoint in GPU VRAM.",
    "theory": {
      "title": "LoRA (Low-Rank Adaptation) Multi-Task Architecture",
      "what_is_it": "LoRA hypothesizes that the weight updates during task adaptation have a low 'intrinsic rank'. Instead of updating W ∈ ℝ^{d × k} directly, it constrains the update by representing ΔW = (α/r) · B · A, where A is initialized with random Gaussian values and B is initialized with zeros (so ΔW is initially 0).\n\n💡 Subject-Related Example:\nA medical center serves an 8B LLaMA model (16 GB VRAM). Instead of loading three separate 16 GB models for Radiology, Cardiology, and Billing (48 GB VRAM total), they freeze the single 16 GB base model and load three 20 MB LoRA adapters, swapping them dynamically per user request!\n\n🚀 Real-World AI Use Case:\nPowering multi-tenant LLM APIs (OpenAI fine-tuning, Hugging Face PEFT, vLLM multi-LoRA serving) where thousands of users have custom fine-tuned adapters running on shared base model clusters.",
      "why_we_need_it": "Full fine-tuning requires copying all model parameters and optimizer states (Adam requires 16 bytes per parameter, so a 70B model needs 1.1 TB VRAM!). LoRA reduces trainable parameters by >99%, drastically slashing memory requirements.",
      "how_it_works": "• Step 1: Base weight matrix W₀ ∈ ℝ^{d × k} is completely frozen (requires no gradients or optimizer states).\n• Step 2: Attach adapter branch: B ∈ ℝ^{d × r} and A ∈ ℝ^{r × k}, where rank r is small (e.g., 4, 8, 16).\n• Step 3: Forward pass computes: h = W₀ · x + (α / r) · (B · A) · x.\n• Step 4: Backpropagation updates only B and A.",
      "formula": "h = W_0 x + \\Delta W x = W_0 x + \\frac{\\alpha}{r} B A x, \\quad \\text{where } B \\in \\mathbb{R}^{d \\times r}, A \\in \\mathbb{R}^{r \\times k}",
      "key_takeaways": [
        "⭐ Frozen Base: Keeps base model weights 100% frozen; only low-rank matrices A and B are trained.",
        "⭐ Multi-Adapter Serving: Allows serving multiple specialized task adapters from a single frozen base model.",
        "⭐ Initialized to Zero: B is initialized to 0, ensuring training starts exactly at the base model's performance."
      ]
    },
    "sample_questions": [
      {
        "q": "Why is matrix B initialized to all zeros in LoRA?",
        "options": [
          "To ensure that ΔW = B · A is initially zero, so adaptation begins exactly at the pre-trained base model state",
          "To make backpropagation faster",
          "To avoid computing gradients for matrix A",
          "To prevent division by zero in Softmax"
        ],
        "ans": "To ensure that ΔW = B · A is initially zero, so adaptation begins exactly at the pre-trained base model state",
        "exp": "Because B = 0, ΔW = B · A = 0 initially, ensuring no degradation of pre-trained capabilities at step 0."
      },
      {
        "q": "Can multiple LoRA adapters be hosted in memory simultaneously on a single base model?",
        "options": [
          "Yes, because base weights are shared and frozen while lightweight adapter matrices are routed dynamically",
          "No, each adapter requires duplicating the base model in VRAM",
          "Only if the base model has fewer than 1 million parameters",
          "Only during training, not during inference"
        ],
        "ans": "Yes, because base weights are shared and frozen while lightweight adapter matrices are routed dynamically",
        "exp": "Multi-LoRA frameworks (like vLLM and LoRAX) serve thousands of fine-tuned adapters concurrently on one frozen base model."
      }
    ]
  },
  {
    ...
    "module_name": "Module 8: Fine-Tuning & Parameter-Efficient Adaptation (LoRA & PEFT)",
    "syllabus_lec": "Lectures 36–37",
    "topic": "LoRA Trainable Parameter Count Calculation (2048 × 1024 at Rank 4)",
    "difficulty": "Medium",
    "points": 1,
    "question": "LoRA is applied to a 2,048 × 1,024 weight matrix using rank r = 4. If only the two low-rank matrices are trainable, how many trainable parameters are added for this weight?",
    "options": [
      "8,192",
      "12,288",
      "16,384",
    ...
    "correct": "12,288",
    "explanation": "In LoRA, a weight matrix W of shape d_in × d_out is decomposed into two low-rank matrices: A of shape d_in × r and B of shape r × d_out (or vice-versa depending on multiplication convention):\n1. Parameters in matrix 1: 2,048 × 4 = 8,192 parameters.\n2. Parameters in matrix 2: 4 × 1,024 = 4,096 parameters.\n3. Total trainable adapter parameters = 8,192 + 4,096 = 12,288 parameters.\nNotice that the original frozen weight has 2,048 × 1,024 = 2,097,152 parameters! LoRA trains only 12,288 / 2,097,152 ≈ 0.58% of the original parameters!",
    "theory": {
      "title": "LoRA Low-Rank Parameter Count Formula & Arithmetic",
      "what_is_it": "LoRA decomposes an update ΔW ∈ ℝ^{d_out × d_in} into B · A, where B ∈ ℝ^{d_out × r} and A ∈ ℝ^{r × d_in}. The total number of trainable parameters in this decomposition is r × (d_in + d_out).\n\n💡 Subject-Related Example:\nFor a linear layer with d_in = 2048, d_out = 1024, and rank r = 4:\n• Matrix A: 4 × 2048 = 8,192 parameters.\n• Matrix B: 1024 × 4 = 4,096 parameters.\n• Sum = 8,192 + 4,096 = 12,288 parameters.\nCompared to the original 2,097,152 parameters, this represents a 170x parameter reduction!\n\n🚀 Real-World AI Use Case:\nCalculated when provisioning GPU clusters for fine-tuning open-source models with Hugging Face PEFT or Unsloth.",
      "why_we_need_it": "Full fine-tuning requires 2,097,152 parameters plus optimizer states (16 bytes per param in FP32 Adam = 33.5 MB for just this single matrix). LoRA needs only 12,288 × 16 bytes = 196 KB, making adaptation feasible on consumer GPUs.",
      "how_it_works": "• Step 1: Identify dimensions: d_in = 2048, d_out = 1024, rank r = 4.\n• Step 2: Compute parameters of matrix A: r × d_in = 4 × 2048 = 8,192.\n• Step 3: Compute parameters of matrix B: d_out × r = 1024 × 4 = 4,096.\n• Step 4: Total trainable params = (r × d_in) + (d_out × r) = 8,192 + 4,096 = 12,288.",
      "formula": "\\text{LoRA Params} = r \\times (d_{\\text{in}} + d_{\\text{out}}) = 4 \\times (2048 + 1024) = 4 \\times 3072 = 12,288",
      "key_takeaways": [
        "⭐ Universal Formula: Trainable Parameters = r × (d_in + d_out).",
        "⭐ Linear vs Quadratic: LoRA parameter count scales linearly with rank r and input/output dimensions, rather than quadratically (d_in × d_out).",
        "⭐ Calculation: 4 × (2048 + 1024) = 4 × 3072 = 12,288."
      ]
    },
    "sample_questions": [
      {
        "q": "If LoRA rank r = 8 is applied to a 4096 × 4096 linear layer, how many trainable parameters are added?",
        "options": [
          "65,536",
          "32,768",
          "16,777,216",
