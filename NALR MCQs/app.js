...
function openInChatGPT(q, qNum, btnElem) {
  ...
  // Open ChatGPT immediately while this function is still running directly
  // from the user's click. This avoids popup blockers caused by waiting for
  // the asynchronous clipboard operation first.
  ...
  const showCopiedState = () => {
    ...
    showToastNotification(
      chatGPTWindow
        ? "✨ Question copied! ChatGPT opened — press Ctrl + V to paste."
        : "✨ Question copied! ChatGPT could not be opened automatically; please allow pop-ups and try again."
    );
  };

  // Copy after opening ChatGPT. Clipboard permission is still initiated by
  // the same user click, while the popup has already been opened.
  if (navigator.clipboard && navigator.clipboard.writeText) {
